...
* /etc/passwd  
  ...
  打开会显示类似的消息：123:x10228:10228::/home/123:/bin/sh

  分别表示用户名  用户ID 组ID 家目录 bash类型，其中x表示密码加密存储在/etc/shadow文件中

* /etc/shadow

  存储加密的密码信息和其他一些信息。

  总共包含8个字段：分别为：**账户：密码：上一次修改密码的时间：密码有效期最少天数：密码有效期最多天数：密码修改警告期：密码非活动期：账号失效天数**

  密码有效期最多天数 = 密码有效期最少天数 + 密码修改警告期
...
* /etc/group
  ...
  存储组名，组ID，同样x 表示密码加密存储于/etc/shadow
...
* UID和GID是一个32位整数
* 任何用户被分配一个唯一的用户ID号 (UID)
* root用户的UID和GID都为0
* 系统用户ID和组ID介于201到999之间
* 组合用户的默认属性定义在头文件/etc/login.defs中
* 
